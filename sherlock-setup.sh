...
# Use: sherlock-setup <MachineName> <URL> [password]
# ===============================================================================
...
sherlock-setup() {
  ...
  for cmd in 7z wget sha256sum; do 
    if ! command -v "$cmd" &>/dev/null; then
      echo -e "\e[31m[!] The command $cmd doesn't exist\e[0m"
      return 1
    fi
  done
  ...
  if [[ -z  $1 || -z $2 ]]; then
    ...
    echo -e "Use: sherlock-setup <\e[33mmachineName\e[0m> <\e[34mhttps://example.com/filename\e[0m> [password]"
  ...
  if wget -q --show-progress -O "$filepath" "$url" ; then
    ...
    if 7z x -bso0 -y -p"$password" "$filepath" -o"$name/artifacts/"; then
      ...
      if [[ "${BASH_SOURCE[0]}" != "${0}" ]]; then
        ...
        echo -e "\e[32m[+] Environment set up at $(pwd)/$name\e[0m"
      fi
